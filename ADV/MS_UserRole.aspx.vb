...
Partial Class MS_UserRole
    ...
    Public Shared Function Delete(ByVal LoginRoleID As String) As String
        ...
        oSqlHelper.CommandText = "SELECT * FROM ADVFIXAM.dbo.MS_UserRoleAssignment WHERE LoginRoleID=@LoginRoleID"
        oSqlHelper.AddParameter("@LoginRoleID", LoginRoleID, SqlDbType.Char)
        dt = oSqlHelper.ExecuteDataTable()
        If dt.Rows.Count > 0 Then
            dict.Add("status", "SDA")
        Else
            oSqlHelper.CommandText = "Delete From ADVFIXAM.dbo.MS_UserRole Where LoginRoleID=@LoginRoleID"
            oSqlHelper.AddParameter("@LoginRoleID", LoginRoleID, SqlDbType.Char)
            Try
